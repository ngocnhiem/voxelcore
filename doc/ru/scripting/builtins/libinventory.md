...
> При разделении стека (ПКМ) данные не копируются а перемещаются в новый стек.
...
```
для перемещения является неэффективным, используйте inventory.move или inventory.move_range.
```lua
